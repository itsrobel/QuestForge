...
def get_llm_opus(streaming=False):
    return ChatAnthropic(model=opus_model_id, temperature=0) if USE_ANTHROPIC_DIRECT else initialize_bedrock_llm(model_id=OPUS_MODEL_ID, streaming=streaming)
def get_llm_sonnet(streaming=False):
    return ChatAnthropic(model=sonnet_model_id, temperature=0) if USE_ANTHROPIC_DIRECT else initialize_bedrock_llm(model_id=SONNET_MODEL_ID, streaming=streaming)

def get_llm_haiku(streaming=False):
    return ChatAnthropic(model=haiku_model_id, temperature=0) if USE_ANTHROPIC_DIRECT else initialize_bedrock_llm(model_id=HAIKU_MODEL_ID, streaming=streaming)
